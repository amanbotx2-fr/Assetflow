...
assetRoutes.get("/", validate({ query: assetListQuerySchema }), assetController.listAssets);
assetRoutes.post("/", requireRoles(Role.ADMIN, Role.MANAGER), validate({ body: createAssetSchema }), assetController.createAsset);
assetRoutes.get("/lookup", validate({ query: assetLookupQuerySchema }), assetController.lookupAsset);
...
assetRoutes.delete("/:id", requireRoles(Role.ADMIN, Role.MANAGER), validate({ params: idParams }), assetController.deleteAsset);
