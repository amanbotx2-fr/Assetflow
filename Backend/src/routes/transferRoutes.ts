...
import { cancelSchema, createTransferSchema } from "../validators/workflowValidators.js";

export const transferRoutes = Router();

transferRoutes.use(authenticate);
transferRoutes.get("/", requireRoles(Role.ADMIN, Role.MANAGER, Role.EMPLOYEE), validate({ query: paginationQuery }), transferController.listTransfers);
...
transferRoutes.get("/:id", requireRoles(Role.ADMIN, Role.MANAGER, Role.EMPLOYEE), validate({ params: idParams }), transferController.getTransfer);
