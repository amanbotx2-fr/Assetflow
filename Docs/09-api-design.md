...
| `GET /assets/:id/qr` | Generate or return QR identity data. | `Authorization` | Admin, Manager, Auditor scoped. | None | `{"qrValue":"assetflow:ast_1","imageUrl":"/uploads/qr/ast_1.png"}` | Valid asset ID. | Not found, forbidden. | `200`, `401`, `403`, `404` | `assets`, `qr_codes` |
...
| `PATCH /transfers/:id/reject` | Reject transfer. | `Content-Type`, `Authorization` | Admin, Manager scoped. | `{"decisionNotes":"Asset still required"}` | `{"id":"trn_1","status":"REJECTED"}` | Transfer must be pending; notes required. | Already decided, forbidden. | `200`, `400`, `401`, `403`, `404` | `transfers`, `audit_logs`, `notifications` |
...
| `PATCH /bookings/:id/reject` | Reject booking. | `Content-Type`, `Authorization` | Admin, Manager scoped. | `{"decisionNotes":"Unavailable"}` | `{"id":"bkg_1","status":"REJECTED"}` | Booking pending; notes recommended. | Already decided. | `200`, `400`, `401`, `403`, `404` | `bookings`, `audit_logs`, `notifications` |
| `PATCH /bookings/:id/cancel` | Cancel booking. | `Content-Type`, `Authorization` | Requester, Admin, Manager scoped. | `{"reason":"No longer needed"}` | `{"id":"bkg_1","status":"CANCELLED"}` | Booking cancellable. | Forbidden, invalid status. | `200`, `400`, `401`, `403`, `404` | `bookings`, `audit_logs`, `notifications` |
...
| Endpoint | Purpose | Headers | Authorization | Example Request | Example Response | Validation | Possible Errors | HTTP Codes | Related Tables |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `GET /audits` | List audit records. | `Authorization` | Admin, Auditor, Manager scoped. | `?result=MISSING` | `{"items":[{"id":"aud_1","result":"MISSING"}]}` | Valid filters and scope. | Forbidden. | `200`, `400`, `401`, `403` | `audit_records`, `assets`, `users` |
